...
const memoryService = __importStar(require("./services/memory"));
const app = (0, express_1.default)();
const PORT = process.env.PORT || 8080;
const contractReviews = new Map();
...
app.get('/', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '..', 'public', 'legal.html'));
});
...
async function sendRealEmail(to, subject, body, fromEmail, appPassword) {
...
app.post('/api/chat/legal', upload.single('file'), async (req, res) => {
    try {
        ...
        const userId = req.headers['x-greennode-agentbase-user-id'] || 'legal-user';
        const sessionId = req.headers['x-greennode-agentbase-session-id'] || `legal-${Date.now()}`;
...
`;
        ...
        if (toolCalls.length > 0) {
            ...
            res.json({
                success: true,
                data: { message: finalMessage, tools_used: toolCalls.map(tc => tc.function?.name).filter(Boolean) },
            });
        }
        else {
...
// ==================== LEGAL HELPERS ====================
async function sendLegalApproval(merchantName, bdEmail, contractVersion, notes) {
    const key = merchantName.toLowerCase();
    ...
    if (existingStatus) {
        ...
        if (contractVersion)
            existingStatus.contractVersion = contractVersion;
...
async function sendLegalRejection(merchantName, bdEmail, reason, reasonCode) {
    const key = merchantName.toLowerCase();
    const existingStatus = contractReviews.get(key);
    if (existingStatus) {
...
async function sendLegalRevision(merchantName, bdEmail, revisionItems, notes) {
    const key = merchantName.toLowerCase();
    const existingStatus = contractReviews.get(key);
    if (existingStatus) {
...
async function summarizeOldHistory(history, openaiClient, model) {
